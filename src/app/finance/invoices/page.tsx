  ...
  if (invoiceError) {
    return (
      <div className="p-12 flex flex-col items-center justify-center text-center bg-rose-50 rounded-3xl border border-rose-100">
        ...
        <h3 className="text-lg font-black text-rose-900 uppercase tracking-tight">Failed to load invoices</h3>
        <p className="text-xs font-bold text-rose-600 mt-2 max-w-xs">{invoiceError.message}</p>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Total Invoices", value: total, icon: Receipt, color: "text-cyan-600", bg: "bg-cyan-50" },
            { label: "Pending Approval", value: pendingCount, icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
            { label: "Settled/Paid", value: paidCount, icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
            { label: "Overdue Alerts", value: overdueCount, icon: AlertCircle, color: "text-rose-600", bg: "bg-rose-50" },
          ].map((item) => (
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
          <div ref={menuRef} className="relative w-full lg:w-auto">
            <button
              ...
              className="w-full lg:w-auto flex items-center justify-between lg:justify-start gap-3 px-5 h-11 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-cyan-800 hover:bg-gray-50 transition-colors shadow-sm"
            >
            ...
            {showColumnMenu && (
              <div className="absolute left-0 mt-2 w-full lg:w-64 bg-white border border-gray-100 rounded-2xl shadow-2xl z-50 p-2 animate-in fade-in slide-in-from-top-2">
                <div className="p-3 mb-2 border-b border-gray-50">
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Toggle Columns</p>
                </div>
                {Object.entries(columnOptions).map(([key, label]) => (
                  <label key={key} className="flex items-center gap-3 px-3 py-2.5 hover:bg-cyan-50 rounded-xl cursor-pointer transition-colors group">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-[11px] font-bold text-gray-600 group-hover:text-cyan-800">{label}</span>
                  </label>
                ))}
              </div>
            )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto no-scrollbar relative animate-in zoom-in-95 duration-300">
