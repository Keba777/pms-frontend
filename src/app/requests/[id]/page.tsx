...
const RequestDetailPage = () => {
  ...
  return (
    <div className="p-4 sm:p-6 bg-white min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-8">
        {[
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {resources.map((res) => (
          res.items.length > 0 && (
            <div key={res.title} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-4">
              <div className="flex items-center justify-between border-b border-gray-50 pb-2">
                <h3 className="text-sm font-black text-cyan-700 uppercase tracking-wider">{res.title}</h3>
                <span className="bg-cyan-50 text-cyan-700 px-2 py-0.5 rounded text-[10px] font-bold">
                  {res.items.length}
                </span>
              </div>
              <ul className="flex flex-col gap-3">
                {res.items.map((item) => (
                  <li key={item.id} className="text-sm font-bold text-gray-700 flex items-center gap-2">
