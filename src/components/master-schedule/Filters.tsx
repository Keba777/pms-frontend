// components/master-schedule/Filters.tsx
...
const Filters: React.FC<FiltersProps> = ({ projects, onChange }) => {
  ...
  const priorities = Array.from(
    new Set(projects.map((p) => p.priority || ""))
  );

  // emit on any change
  useEffect(() => {
    onChange({ status, priority, startDate, endDate });
  }, [status, priority, startDate, endDate, onChange]);
