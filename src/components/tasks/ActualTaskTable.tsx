...
const ActualTaskTable: React.FC = () => {
  ...
  // define columns for customization, including Remaining and Budget
      ...
      <div className="mt-2 mb-6">
        <GenericDownloads<Task>
          ...
          columns={Object.entries(columnOptions)
            ...
            .map(([key, label]) => {
              // For Remaining and Budget, use accessor functions
              if (key === "remaining") {
                return {
                  header: label,
                  accessor: (row: Task) =>
                    getDuration(new Date().toISOString(), row.end_date),
                };
              }
              if (key === "budget") {
                return {
              ...
              // Default to direct Task field for others
              return {
                header: label,
